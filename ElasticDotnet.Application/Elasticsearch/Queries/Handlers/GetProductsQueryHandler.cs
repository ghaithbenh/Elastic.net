...
public sealed class GetProductsQueryHandler : IRequestHandler<GetProductsQuery, ISearchResponse<AxamProduct>>
{
    ...
    public GetProductsQueryHandler(IElasticClient elasticClient, ISender sender, IKeywordRepository keywordRepository)
    {
    ...
    {
        await _keywordRepository.AddKeywordAsync(request.SearchQuery);
        ...
        var response = await KnnSearchAsync<AxamProduct>(encodedQuery, request.KnnSearchRequest);
